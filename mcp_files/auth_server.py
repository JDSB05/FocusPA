...
def create_authorization_server(server_settings: AuthServerSettings, auth_settings: SimpleAuthSettings) -> Starlette:
    ...
    async def login_page_handler(request: Request) -> Response:
        ...
        return JSONResponse({"state": state, "redirect_uri": f"{server_settings.server_url}login/callback"})
    ...
    async def introspect_handler(request: Request) -> Response:
        ...
        # print all the request data for debugging
        print("DEBUG_003")
        print(f"access_token.resource = {access_token.resource}")
        print("DEBUG_003")
...
@click.command()
@click.option("--port", default=9001, help="Port to listen on")
