...
users = {
    "admin": {"password": "admin", "scopes": ["user", "admin"]},
    "user": {"password": "user", "scopes": ["user"]},
}

def get_user_scopes(username: str, password: str) -> list[str]:
    """Get scopes for a given username."""
...
class SimpleOAuthProvider(OAuthAuthorizationServerProvider[AuthorizationCode, RefreshToken, AccessToken]):
    ...
    async def handle_simple_callback(self, username: str, password: str, state: str) -> str:
        ...
        # !!! TODO: Substituir isto por algo mais seguro !!!
        allowed_scopes = get_user_scopes(username, password)
    ...
    async def exchange_authorization_code(
        ...
        print("DEBUG_002")
        print("I just issued a token: ", mcp_token)
        print("DEBUG_002")
